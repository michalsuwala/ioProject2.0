...
public class Individual {
    ...
    public static void addIndividual(int SpeciesId, String description) throws SQLException {
        ...
        String checkIfSpeciesIdExists = String.format("SELECT EXISTS(SELECT * FROM database.Species WHERE species_id = %d) AS \"EXISTS\"", SpeciesId);
        try (Statement stmt = Main.conn.createStatement()) {
            ...
            if(speciesExist) {
                ...
                String insertIndividualDataToDB = String.format("INSERT INTO database.Individual (individual_id, description, species_id) VALUES (%d, '%s', %d)", maxIndividualId + 1, description, SpeciesId);
